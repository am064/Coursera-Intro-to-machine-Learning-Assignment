...
J=0;
for i=1:m
  J1=J1+(((-y1(:,i)'))*log((H(:,i)))-((1-y1(:,i))')*(log(1-H(:,i))));
end
...
J=(J1/m)+(lambda/(2*m))*(sum(sum(Theta1(:,2:end).^2))+sum(sum(Theta2(:,2:end).^2)));
    
% Backpropogation Algorithm for 3 layers
delta_1=0;
delta_2=0;
for t=1:m
X1=[ones(1,1);(X(t,:)')];
